...
当源材料是书籍、教材集、指南库、证据库或数据库时，先做 source map / schema map，再拆 expert。不要从长正文改写开始。
...
| 边类型 | 何时打开 | 例 |
|---|---|---|
| schema prerequisite | 解释字段前必须知道单位/编码 | nutrient_value -> unit / missing code |
| provenance anchor | 需要知道数据来源与版本 | food item -> table edition / source page |
| normalization action | 输出需统一单位或分类 | mg/100g -> serving-level estimate |
| safety/evidence | 解释会影响健康建议 | nutrient threshold -> guideline gate |
